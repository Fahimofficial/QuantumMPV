...
object MusicLibraryScanner {
  ...
  suspend fun scanSongs(context: Context): List<MusicSong> =
        ...
        context.contentResolver
          .query(
            ...
            while (cursor.moveToNext()) {
              ...
              val artist =
                cursor.getString(artistCol)?.takeIf { it.isNotBlank() && it != "<unknown>" } ?: "Unknown Artist"
  ...
  suspend fun scanAlbums(
    ...
    withContext(Dispatchers.IO) {
      if (songs.isNotEmpty()) {
        ...
        songs
          .groupBy { if (it.albumId > 0) it.albumId else it.album.hashCode().toLong() }
