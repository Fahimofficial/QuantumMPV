...
class JellyfinRepository(
  ...
  suspend fun authenticate(
    ...
    username: String,
    password: String,
  ): Result<JellyfinAuthResult> = client.authenticate(serverUrl, username, password)

  suspend fun validateToken(
    serverUrl: String,
    token: String,
  ): Result<JellyfinUser> = client.validateToken(serverUrl, token)
  ...
  suspend fun getResumeItems(
    ...
    limit: Int = 16,
  ): Result<List<JellyfinItem>> = client.getResumeItems(server.serverUrl, server.userId, server.accessToken, limit)
  ...
  suspend fun getSuggestions(
    server: JellyfinServer,
    limit: Int = 16,
  ): Result<List<JellyfinItem>> = client.getSuggestions(server.serverUrl, server.userId, limit, server.accessToken)
  ...
  suspend fun getItem(
    server: JellyfinServer,
    itemId: String,
  ): Result<JellyfinItem> = client.getItem(server.serverUrl, server.userId, itemId, server.accessToken)

  suspend fun deleteItem(
    server: JellyfinServer,
    itemId: String,
  ): Result<Unit> = client.deleteItem(server.serverUrl, itemId, server.accessToken)
  ...
  suspend fun getGenres(
    server: JellyfinServer,
    parentId: String,
  ): Result<List<String>> = client.getGenres(server.serverUrl, server.userId, parentId, server.accessToken)

  suspend fun getSeasons(
    server: JellyfinServer,
    seriesId: String,
  ): Result<List<JellyfinItem>> = client.getSeasons(server.serverUrl, server.userId, seriesId, server.accessToken)
  ...
  suspend fun getEpisodes(
    ...
    client.getEpisodes(server.serverUrl, server.userId, seriesId, seasonId, server.accessToken)

  suspend fun getSubtitleTracks(
    server: JellyfinServer,
    itemId: String,
  ): Result<List<PlaybackSubtitleTrack>> =
    client.getSubtitleTracks(server.serverUrl, server.accessToken, server.userId, itemId)
  ...
  fun getImageUrl(
    ...
    val targetItemId =
      if (item.primaryImageTag.isNullOrBlank() &&
        ...
        !item.albumPrimaryImageTag.isNullOrBlank()
      ) {
        item.albumId
      } else {
        item.id
  ...
  private suspend fun decryptAndMigrate(entity: JellyfinServerEntity): JellyfinServer {
    ...
    val token =
      if (credentialCipher.isEncrypted(entity.accessToken)) {
        try {
          credentialCipher.decrypt(entity.accessToken)
        } catch (
          e: Exception,
        ) {
  ...
  private fun JellyfinServer.toStorageEntity(): JellyfinServerEntity =
    JellyfinServerEntity.fromDomain(this).let { entity ->
      entity.copy(accessToken = encryptForStorage(entity.accessToken))
    }
