...
object EmbeddedLyricsExtractor {
  ...
  private fun findLyricsDirectlyFromMedia(
    context: Context,
    mediaPath: String,
  ): String? {
    return runCatching {
    ...
    try {
      ...
      if ((tagFlags and 0x40) != 0) {
        if (majorVersion == 4 && offset + 4 <= tagSize) {
          val extSize =
            ((tagData[offset].toInt() and 0x7F) shl 21) or
      ...
      while (offset + 10 <= tagSize) {
        ...
        val (headerLen, frameSize) =
          if (majorVersion == 2) {
            val size =
              ((tagData[offset + 3].toInt() and 0xFF) shl 16) or
        ...
        if (frameId == "USLT" || frameId == "ULT") {
          ...
          while (contentOffset < frameEnd) {
            if (nullTermSize == 2) {
              if (contentOffset + 1 < frameEnd &&
                tagData[contentOffset] == 0.toByte() &&
                tagData[contentOffset + 1] == 0.toByte()
              ) {
                contentOffset += 2
                break
              }
