# QuantumMPV Lua and JavaScript Command Guide

This file documents the public scripting surface that QuantumMPV exposes to mpv Lua
and JavaScript scripts.

Scripts talk to QuantumMPV by writing string values to properties under:

```text
user-data/mpvrx/*
```

QuantumMPV observes those properties, performs the native player action, and clears
...
1. Enable Lua/JS scripting in QuantumMPV advanced settings.
2. Put script files in the selected mpv config folder.
3. Prefer the `scripts/` subfolder. QuantumMPV also falls back to the config root.
4. Use `.lua` for Lua scripts and `.js` for JavaScript scripts.
5. Select the scripts in the QuantumMPV scripts panel.
6. Reopen the video if a script was added before playback started.

QuantumMPV also syncs `script-opts/` from the selected mpv config folder.

## Important Rules

- Every QuantumMPV command property is handled as a string.
...
| --- | --- | --- |
| `user-data/mpvrx/show_text` | Any string | Shows a native QuantumMPV text overlay. |
| `user-data/mpvrx/toggle_ui` | `show`, `hide`, `toggle` | Shows, hides, or toggles player controls. |
| `user-data/mpvrx/show_panel` | Panel id | Opens a native QuantumMPV sheet or panel. |
...
| `user-data/mpvrx/curl_response` | JSON string | Response written by QuantumMPV. Scripts should observe it. |
...
key bindings for common QuantumMPV commands.
...
function createPost() {
    var payload = {
        title: "QuantumMPV JavaScript curl test",
        body: "Posted from an mpv JavaScript script through QuantumMPV curl.",
        userId: 1
    };
...
Shows a short native QuantumMPV overlay.
...
Opens a native QuantumMPV sheet or panel.
...
QuantumMPV writes the response to:
...
| --- | --- | --- | --- | --- |
| `id` | string | No | UUID generated by QuantumMPV | Use your own id so scripts can match responses. |
...
QuantumMPV custom buttons support Lua and JavaScript actions.
...
Paste only the action body into the editor. QuantumMPV wraps it in a generated
...
- QuantumMPV writes generated scripts to the app internal `scripts/` directory.
...
QuantumMPV writes Android device state into mpv `user-data/android/*` properties.
...
- Confirm scripting is enabled in QuantumMPV settings.
